...
export default class MarkdownViewerPrefs extends ExtensionPreferences {
    fillPreferencesWindow(window) {
        const settings = this.getSettings('org.gnome.shell.extensions.markdown-viewer');
        ...
        const startSpin = new Gtk.SpinButton({
            adjustment: new Gtk.Adjustment({ lower: 1, upper: 9999, step_increment: 1 }),
            numeric: true,
        });
        ...
        const endSpin = new Gtk.SpinButton({
            adjustment: new Gtk.Adjustment({ lower: 0, upper: 9999, step_increment: 1 }),
            numeric: true,
        });
